...
class MessagingController extends Controller
{
    ...
    /**
     * List conversations for the authenticated user (customer).
     */
    public function index(Request $request): JsonResponse
    {
        try {
            ...
            Log::error('USER MESSAGING: ' . $e->getMessage());
            return ShopittPlus::response(false, 'Failed to list conversations', 500);
        }
    }

    /**
     * Get or create driver-customer conversation for an order.
    ...
    public function getOrCreateWithDriver(Request $request): JsonResponse
    {
        ...
        try {
            /** @var User $user */
            $user = Auth::user();
            $order = Order::with('driver')->findOrFail($data['order_id']);

            if ($order->user_id !== $user->id) {
                return ShopittPlus::response(false, 'Order does not belong to you', 403);
            }
            ...
            Log::error('USER MESSAGING CREATE: ' . $e->getMessage());
    ...
    public function messages(string $conversationId, Request $request): JsonResponse
    {
        try {
            ...
            Log::error('USER MESSAGING MESSAGES: ' . $e->getMessage());
    ...
    public function send(Request $request, string $conversationId): JsonResponse
    {
        ...
        try {
            ...
            Log::error('USER MESSAGING SEND: ' . $e->getMessage());
