...
class CheckOrderManagementScope
{
    /**
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        // Check if admin has wildcard privilege or order-management scope
        if (in_array('*', $permissions) || in_array('order-management', $permissions)) {
            return $next($request);
        }

        return ShopittPlus::response(false, 'Access denied. Order management privilege required.', 403);
    }
}
