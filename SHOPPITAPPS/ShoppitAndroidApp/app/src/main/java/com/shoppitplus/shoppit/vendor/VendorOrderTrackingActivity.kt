...
class VendorOrderTrackingActivity : AppCompatActivity() {
    private lateinit var binding: ActivityOrderTrackingBinding
    private var orderId: String = ""
    private val tag = "VendorOrderTracking"
    ...
    private fun loadTracking() {
        ...
        Log.d(tag, "Loading vendor tracking for order $orderId")
        ...
        lifecycleScope.launch {
            try {
                val api = RetrofitClient.instance(this@VendorOrderTrackingActivity)
                val trackingResponse = api.getVendorOrderTracking(orderId)
                val etaResponse = api.getVendorOrderEta(orderId)
                ...
                if (trackingResponse.isSuccessful) {
                    ...
                    if (!avatarUrl.isNullOrEmpty()) {
                        Glide.with(this@VendorOrderTrackingActivity)
                            .load(avatarUrl)
